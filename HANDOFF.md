...
#### 1. **職種・資格・研修の大幅拡充**
- **国家資格の完全対応** (`types/qualifications.ts`)
  - 介護・福祉系（介護福祉士、社会福祉士、精神保健福祉士）
  - 心理・保育系（公認心理師、保育士）
  - 医療・リハビリ系（医師、看護師、PT/OT/ST、管理栄養士、薬剤師など19資格）

- **公的資格・法定研修の完全対応**
  - 介護保険（初任者研修、実務者研修、ケアマネ、認知症介護研修、喀痰吸引等研修など）
  - 障害福祉（サビ管、児発管、相談支援専門員、強度行動障害支援者研修、同行援護など）

- **民間資格**（臨床心理士、福祉住環境コーディネーター、認知症ケア専門士など）

- **介護保険・障害福祉サービス種別** (`types/services.ts`)
  - 介護保険サービス全種別（居宅、地域密着型、施設、計画・支援）
  - 障害福祉サービス全種別（訪問系、日中活動、施設、相談支援、児童、地域生活支援）

#### 2. **スタッフ管理の複数選択機能**
- **職種の複数選択対応** (`types/staff.ts`)
  - `jobType` (単一) → `jobTypes` (配列) に変更
  - `qualifications` (資格配列) を新規追加

- **MultiSelectCheckbox コンポーネント** (`components/MultiSelectCheckbox.tsx`)
  - 検索機能付き複数選択チェックボックス
  - すべて選択/クリア機能
  - 選択済みアイテムのタグ表示

- **スタッフ新規登録フォームの更新** (`app/staff/new/page.tsx`)
  - 職種・資格の複数選択対応
  - AuthContext からユーザー・組織情報を取得
  - バリデーション強化

- **スタッフ一覧の表示改善** (`app/staff/page.tsx`)
  - 複数職種をタグ表示
  - 検索機能で複数職種に対応

#### 3. **料金プラン管理**  (`types/organization.ts`)
- **デモプラン**: 無料、職員5人まで、利用者10人まで、データ保存30日
- **サブスクリプションプラン**:
  - 20人以下: 基本料金 2,000円/月
  - 21人以上: 基本料金 2,000円 + 10人ごとに 1,000円
  - 月額計算関数 `calculateMonthlyPrice()` を実装
...
- **Organization型の拡張**:
  ...
  - `stripeCustomerId`, `stripeSubscriptionId` (将来の決済API統合用)
...
#### 1. **レスポンシブデザインの改善**
- [ ] スタッフ一覧・利用者一覧のモバイル対応（テーブル → カードビュー）
- [ ] フォームのモバイル最適化
- [ ] ナビゲーションのモバイル対応強化
- [ ] タッチ操作の最適化

#### 2. **初回セットアップフローの改善**
- [ ] 代表者1人登録からスタートする初回フロー
- [ ] 事業所登録 → 代表者登録の2ステップ
- [ ] デモプランからスタート（デフォルト）
- [ ] サブスクリプションプランへのアップグレード導線

#### 3. **パフォーマンス改善**
- [ ] ページネーション機能の実装（スタッフ一覧、利用者一覧）
- [ ] 仮想スクロール（大量データ対応）
- [ ] 遅延読み込み（Lazy Loading）
- [ ] インデックス最適化（Firestore）
